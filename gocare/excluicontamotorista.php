...
if(!empty($_POST))
{
	...
	if ($senha==$row['senha']){
		...
		$sqldelfoto = "SELECT fotoperfil FROM motorista WHERE email = '$login';";
		$querydelfoto = $db->query($sqldelfoto);
		foreach($querydelfoto as $nome_foto)

		$sql = "DELETE FROM motorista WHERE email = '$login';";
		$query = $db->query($sql);

		// Removendo imagem da pasta fotos/
		if ($nome_foto['fotoperfil']!=='motoristadefault.png'){
		...
		if ($query){
			session_destroy();
			header("Location: perfilmotoristaexcluido.php");
			exit;
		} else {
			header("Location: perfilmotoristaexcluir.php?msg=2");
			exit;
		}
	} else {
	header("Location: perfilmotoristaexcluir.php?msg=3");
	exit;
	}
}
?>
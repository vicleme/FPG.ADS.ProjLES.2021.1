...
if(!empty($_POST))
{
	...
	$conferesenhaantiga = "SELECT senha FROM passageiro WHERE email= '$login';";
	...
	if ($senhaantiga==$row['senha']){
		if($novasenha==$confnovasenha){
			...
			if ($query){
				header("Location: perfilpassageiroeditasenha.php?msg=1");
				exit;
			} else {
				header("Location: perfilpassageiroeditasenha.php?msg=2");
				exit;
			}
		} else {
			header("Location: perfilpassageiroeditasenha.php?msg=4");
			exit;
		}
	} else {
		header("Location: perfilpassageiroeditasenha.php?msg=3");
		exit;
	}
}
?>